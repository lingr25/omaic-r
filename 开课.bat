...
if %errorlevel%==0 (
    ...
    start "FunASR语音识别-勿关闭" cmd /k funasr-server --device cpu --model sensevoice --host 127.0.0.1 --port 8000
)
...
if %errorlevel%==0 (
    ...
    start "OpenMAIC课堂-勿关闭" cmd /k "cd /d %~dp0 && call pnpm dev"
)
